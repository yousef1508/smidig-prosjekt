...
class VolatilityApp(ctk.CTk):
    def __init__(self):
        ...
        header.grid(row=0, column=0, columnspan=2, padx=10, pady=40, sticky="n")
        file_button = ctk.CTkButton(self, text="Select File", command=self.select_file, fg_color="#E46F2E",
                                    text_color=self.text_dark, hover_color="#5A9", font=self.font, width=120)
    ...
    def create_result_tab(self, plugin, renderer, content):
        ...
        save_button = ctk.CTkButton(tab_frame, text="Export", command=lambda: self.save_results(content),
                                     ...
                                     hover_color=self.header_color, font=self.font, width=120)
